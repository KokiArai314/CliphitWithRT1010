...
typedef enum _usb_device_status
{
	...
	/// @note support SOF
	kUSB_DeviceStatusSetInterface,  /*!< Device set interface */
...
/// @note FEEDBACK Info	(Feedback_EP_16ms_Interval)
#define FEEDBACK_REFRESH				4						// 1ms x 16 = 16ms
#define FEEDBACK_INTERVAL_COUNT			(1 << FEEDBACK_REFRESH)

#define HS_FEEDBACK_REFRESH				7						// 125us x 8 x 16 = 16ms
#define HS_FEEDBACK_INTERVAL_COUNT		(1 << HS_FEEDBACK_REFRESH)
#define HS_AUDIOIN_INTERVAL_SOF_COUNT	8						// 125us x 8 = 1ms
...
typedef enum _usb_device_event
{
	...
	kUSB_DeviceEventGetOtherSpeedConfigurationDescriptor,	/*!< Get other speed configuration descriptor. */
...
#if ((defined(USB_DEVICE_CONFIG_EHCI)) && (USB_DEVICE_CONFIG_EHCI > 0U))
extern void USB_DeviceEhciAttachedDevice(void *deviceHandle);
extern uint8_t USB_DeviceEhciGetAttachedDeviceStatus(void);
extern void USB_DeviceEhciMaskUsbInterruptEnable(void* deviceHandle, uint8_t enable);
#endif
