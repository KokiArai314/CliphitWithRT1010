...
//#if (defined(USB_DEVICE_CONFIG_CV_TEST) && (USB_DEVICE_CONFIG_CV_TEST > 0U))
    else if (USB_DESCRIPTOR_TYPE_DEVICE_QUALITIER == descriptorType)
    {
        ...
        error = USB_DeviceClassCallback(classHandle->handle, kUSB_DeviceEventGetDeviceQualifierDescriptor,
                                        &commonDescriptor.deviceQualifierDescriptor);
    }
//#endif
...
#endif
    else if (USB_DESCRIPTOR_TYPE_OTHER_SPEED_CONFIGURATION == descriptorType) {
        /* Get the other speed configuration descriptor */
        error = USB_DeviceClassCallback(classHandle->handle, kUSB_DeviceEventGetOtherSpeedConfigurationDescriptor,
                                        &commonDescriptor.otherSpeedConfigurationDescriptor);
    }
    else
    {
    }
...
#if 1	/// @note support SOF
    {
    	uint16_t setWValueWIndex;	/// @note AudioRec : 引き渡し先でwValue,wIndexが必要: 下位8bytes情報があればOK

        setWValueWIndex = (uint16_t)(((setup->wIndex & 0xFFU) << 8U) | (setup->wValue & 0xFFU));
        USB_DeviceSetStatus(classHandle->handle, kUSB_DeviceStatusSetInterface, &setWValueWIndex);
    }
#endif
