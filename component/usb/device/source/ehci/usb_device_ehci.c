...
#include "board.h" /// @note : #if SUPPORT_USB_HIGH_SPEED
...
static void USB_DeviceEhciCancelControlPipe(usb_device_ehci_state_struct_t *ehciState, uint8_t endpoint,
                                            uint8_t direction);
...
static usb_status_t USB_DeviceEhciTransfer(usb_device_ehci_state_struct_t *ehciState, uint8_t endpointAddress,
...
#else
  /// @note Absolutely necessary
  ehciState->registerBase->PORTSC1 |= USBHS_PORTSC1_PFSC_MASK;
#endif
      ...
      if (status & ((uint32_t)(1U << count))) {
        ...
        while (currentDtd) {
          ...
          while (currentDtd) {
            ...
            currentDtd =
                (usb_device_ehci_dtd_struct_t *)(currentDtd->nextDtdPointer & USB_DEVICE_ECHI_DTD_POINTER_MASK);
          }
 ...
 */
/// @note support SOF
static uint32_t IntSofCount;
extern void USB_AudioFeedbackDataUpdate(void);
extern uint8_t startLRClockCount;

/// @note AudioRec
extern void USB_prepareAudioRecData(void);

static void USB_DeviceEhciInterruptSof(usb_device_ehci_state_struct_t *ehciState) {
#if SUPPORT_USB_HIGH_SPEED
  if (ehciState->speed == USB_SPEED_HIGH) {
    /// @note support SOF
    IntSofCount++;
    if (IntSofCount >= HS_FEEDBACK_INTERVAL_COUNT) {
      IntSofCount = 0;
      USB_AudioFeedbackDataUpdate();
    }
    /// @note add AudioRec + MIDI IN Timeout(1ms)
    /* Prepare USB Audio IN data */
    if ((IntSofCount % HS_AUDIOIN_INTERVAL_SOF_COUNT) == 0) { /// @note 125us x HS_AUDIOIN_INTERVAL_SOF_COUNT = 1ms
      USB_prepareAudioRecData();
    }
  } else {
    /// @note support SOF
    IntSofCount++;
    if (IntSofCount >= FEEDBACK_INTERVAL_COUNT) {
      IntSofCount = 0;
      USB_AudioFeedbackDataUpdate();
    }
    /// @note add AudioRec + MIDI IN Timeout(1ms)
...
static usb_status_t USB_DeviceEhciTransfer(usb_device_ehci_state_struct_t *ehciState, uint8_t endpointAddress,
  ...
  uint32_t index = ((endpointAddress & USB_ENDPOINT_NUMBER_MASK) << 1U) |
                   ((endpointAddress & USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_MASK) >>
                    USB_DESCRIPTOR_ENDPOINT_ADDRESS_DIRECTION_SHIFT);
  volatile uint32_t primeBit = 1U << ((endpointAddress & USB_ENDPOINT_NUMBER_MASK) +
  ...
  /// @note check->USB_DeviceEhciTransfer : do ~ while : always 200ns
  do {
  ...
  if (!qhIdle) {
    ...
    /* To safely a dtd */
    /// @note check->USB_DeviceEhciTransfer : while(waitingSafelyAccess) : 0
    while (waitingSafelyAccess) {
 ...
 * @return A USB error code or kStatus_USB_Success.
 */
usb_status_t USB_DeviceEhciInit(uint8_t controllerId, usb_device_handle handle,
 ...
 * callback).
 */
usb_status_t USB_DeviceEhciSend(usb_device_controller_handle ehciHandle, uint8_t endpointAddress, uint8_t *buffer,
                                uint32_t length) {
  /* Add dtd to the QH */
 ...
 * callback).
 */
usb_status_t USB_DeviceEhciRecv(usb_device_controller_handle ehciHandle, uint8_t endpointAddress, uint8_t *buffer,
                                uint32_t length) {
  /* Add dtd to the QH */
  return USB_DeviceEhciTransfer(
      (usb_device_ehci_state_struct_t *)ehciHandle,
...
usb_status_t USB_DeviceEhciControl(usb_device_controller_handle ehciHandle, usb_device_control_type_t type,
                                   void *param) {
  usb_device_ehci_state_struct_t *ehciState = (usb_device_ehci_state_struct_t *)ehciHandle;
  ...
  switch (type) {
        ...
        if (ep < USB_DEVICE_CONFIG_ENDPOINTS) {
          if (ep) {
            endpointStatus->endpointStatus =
                (ehciState->registerBase->EPCR[ep - 1U] & (direction ? USBHS_EPCR_TXS_MASK : USBHS_EPCR_RXS_MASK))
    ...
    case kUSB_DeviceControlSetDeviceAddress:
      ...
      break;
    /// @note support SOF
    case kUSB_DeviceControlSetInterface:
      /// @note SOF割り込みを許可/禁止する
      /// @note add AudioRec
      /// @note support composite MIDI
      if (g_composite.audioPlayer.attach == 0) {
        break;
      }
      if (param) {
        uint8_t interface;
        uint8_t altSetting;
        uint8_t resetSofParam;
        enum itfNumber { interface_audio_play = 1, interface_audio_rec };
        ...
        resetSofParam = 0;
        switch (interface) {
          case interface_audio_play:
            if (altSetting == 0) {
              if (g_composite.audioPlayer.currentInterfaceAlternateSetting[interface_audio_rec] == 0) {
                /* disable SOF INT */
                ehciState->registerBase->USBINTR &= ~(1 << 7);
              }
            } else {
              if (g_composite.audioPlayer.currentInterfaceAlternateSetting[interface_audio_rec] == 0) {
                /* enable SOF INT */
            ...
            break;
          case interface_audio_rec:
            if (altSetting == 0) {
              if (g_composite.audioPlayer.currentInterfaceAlternateSetting[interface_audio_play] == 0) {
                /* disable SOF INT */
                ehciState->registerBase->USBINTR &= ~(1 << 7);
              }
            } else {
        ...
        if (resetSofParam == 1) {
          /// @note reset SOF Counter for USB_AudioFeedbackDataUpdate() in audio_player.c
          IntSofCount = 0;
          startLRClockCount = 0;
        }
      }
...
void USB_DeviceEhciIsrFunction(void *deviceHandle) {
  ...
  /// @note SOF Interrupt > Token done interrupt
...
/**
 * i.MX RT1020 USB_FRINDEX register
 * USB_FRINDEX_regの値に変化があれば, USBデバイスとして認識されている
 */
#define DETACH_COUNT_MS_MAX 5 // (100ms x 5 = 500ms目安)
static uint16_t detach_status_ms_count;
static uint32_t keep_USB_FRINDEX_reg;
static uint8_t attach_status;
static uint8_t startCheckAttachedStatus = 1;
void USB_DeviceEhciAttachedDevice(void *deviceHandle) {
  if (NULL != deviceHandle) {
    usb_device_struct_t *handle = (usb_device_struct_t *)deviceHandle;
    usb_device_ehci_state_struct_t *ehciState;
    uint32_t USB_FRINDEX_reg;

    ehciState = (usb_device_ehci_state_struct_t *)(handle->controllerHandle);
    USB_FRINDEX_reg = ehciState->registerBase->FRINDEX;

    if (startCheckAttachedStatus) {
      startCheckAttachedStatus = 0;
      keep_USB_FRINDEX_reg = USB_FRINDEX_reg;
      return;
    }

    if (USB_FRINDEX_reg != keep_USB_FRINDEX_reg) {
      attach_status = 1; // attached
      detach_status_ms_count = 0;
      keep_USB_FRINDEX_reg = USB_FRINDEX_reg;
    } else {
      if (detach_status_ms_count < DETACH_COUNT_MS_MAX) {
        detach_status_ms_count++;
      } else {
        if (attach_status == 1) {
          attach_status = 0;
          /* SOF Reset */
...
uint8_t USB_DeviceEhciGetAttachedDeviceStatus() { return attach_status; }

void USB_DeviceEhciMaskUsbInterruptEnable(void *deviceHandle, uint8_t enable) {
  if (NULL == deviceHandle) {
    return;
  } else {
    usb_device_struct_t *handle = (usb_device_struct_t *)deviceHandle;
    ...
    ehciState = (usb_device_ehci_state_struct_t *)(handle->controllerHandle);
    if (enable == 1) {
      /* USB interrupt Enable (UE)->1 */
      ehciState->registerBase->USBINTR |= (1 << 0);
    } else {
      /* USB interrupt Disable (UE)->0 */
      ehciState->registerBase->USBINTR &= ~(1 << 0);
    }
  }
}
