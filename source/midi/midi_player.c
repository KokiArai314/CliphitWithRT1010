...
static const struct UsbMidiDecodeInfo {
  ...
  uint8_t size;
    ...
    {TYPE_IGNORE,
     0}, /*	0x0:	Miscellaneous function codes. Reserved for future extensions.				*/
    {TYPE_IGNORE,
     0}, /*	0x1:	Cable events.Reserved for future expansion.									*/
    {TYPE_PASS,
    ...
    {TYPE_SYSEX_COMMON_END,
     1}, /*	0x5:	Single-byte System Common Message SysEx ends with following single byte.	*/
    {TYPE_SYSEX_END,
  ...
  if ((codeID & 0xF0) == 0) {
    ...
    switch (codeID) {
      /*
		...
		*/
      case 0x04: /* SysEx starts or continues */
        midi_AMP_RxProc_SysEx((uint8_t)(UsbMidiData >> 8), (uint8_t)(UsbMidiData >> 16), (uint8_t)(UsbMidiData >> 24));
        break;
      case 0x05: /* Single-byte System Common Message or SysEx ends with following single byte */
      case 0x0F: /* Single Byte */
...
static void USB_MIDI_OUT_IDLE(void) {
  ...
  if (finished == true) {
    ...
    /*　受信できる状態をセット */
    {
      error = USB_DeviceMidiRecv(g_deviceComposite->midiPlayer.midiHandle,
...
static void sendFill0Data(void) {
  if (g_deviceComposite->midiPlayer.txDataTransmissionCompleted == 1) {
    ...
    {
      error = USB_DeviceMidiSend(g_deviceComposite->midiPlayer.midiHandle, (uint8_t *)zeroData,
                                 g_deviceComposite->midiPlayer.currentStreamMaxPacketSize);
      if (error == kStatus_USB_Success) {
        /* 準備できたらフラグをクリア */
...
#if SUPPORT_USB_HIGH_SPEED
  size = (g_deviceComposite->midiPlayer.speed == USB_SPEED_HIGH) ? HS_USB_DEVICE_MIDI_TX_BUFF_DWORD_SIZE
                                                                 : FS_USB_DEVICE_MIDI_TX_BUFF_DWORD_SIZE;
#else
...
uint8_t Send2USB(void) {
  ...
  size = (g_deviceComposite->midiPlayer.speed == USB_SPEED_HIGH) ? HS_USB_DEVICE_MIDI_TX_BUFF_DWORD_SIZE
                                                                 : FS_USB_DEVICE_MIDI_TX_BUFF_DWORD_SIZE;
  fifoSize = (g_deviceComposite->midiPlayer.speed == USB_SPEED_HIGH) ? HS_USB_DEVICE_MIDI_TX_FIFO_DWORD_SIZE
                                                                     : FS_USB_DEVICE_MIDI_TX_FIFO_DWORD_SIZE;
#else
  size = FS_USB_DEVICE_MIDI_TX_BUFF_DWORD_SIZE;
      ...
      if ((g_deviceComposite->midiPlayer.txDataTransmissionCompleted == 1) &&
        ...
        while (g_deviceComposite->midiPlayer.fifoTxBuffer->Cnt > 0) {
          if (g_deviceComposite->midiPlayer.txBuffer->Cnt > 0) {
            g_deviceComposite->midiPlayer.fifoTxBuffer->Buffer[i++] =
                *(g_deviceComposite->midiPlayer.txBuffer->Buffer +
                  g_deviceComposite->midiPlayer.txBuffer->Rd); //usbの出力バッファにMidiを投げ込む
        ...
        if (sendOK) {
          ...
          }
          {
            ...
            error = USB_DeviceMidiSend(g_deviceComposite->midiPlayer.midiHandle,
...
int isMidiInOpen(void) { return MidiInOpen; }
