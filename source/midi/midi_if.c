...
void BOARD_UART_IRQ_HANDLER() {
  ...
  if ((uart_stat & kLPUART_RxDataRegFullFlag) != 0) {
    ...
    while (count > 0) { /* If new data arrived. */
...
void midi_IF_TxProc_ReceiveByte(uint8_t Data) { s_MidiIfTxBuff[s_MidiIfTxNumOfData++] = Data; }
    ...
    if (midiShortEventCount > 0) {
      ...
      setCount = 0;
      {
        ...
        for (i = 0; i < bufSize; i++) {
          ...
          curBuf = ((curBuf + 4) >= (UsbSysExMessageBuf + NUM_OF_USB_MIDI_SYSEX_EVENT_DATA_SIZE)) ? UsbSysExMessageBuf
                                                                                                  : curBuf + 4;
          WriteData4(usbMidiData);
        }
...
#ifdef LOCAL_DEBUG_ENABLE
uint32_t getrxoverrun(void) { return rxoverrun; }
uint32_t getrxbuffovr(void) { return rxbuffovr; }
#endif
